...
def timegan (ori_data, parameters):
  ...
  for itt in range(iterations):
    ...
    embedding_losses.append([itt, np.round(np.sqrt(step_e_loss), 4)])
  ...
  for itt in range(iterations):
    ...
    # Append loss values to the list
    supervised_losses.append([itt, np.round(np.sqrt(step_g_loss_s), 4)])
  ...
  for itt in range(iterations):
    ...
    # Append loss values to the list
    joint_losses.append([itt, np.round(step_d_loss, 4), np.round(step_g_loss_u, 4), np.round(np.sqrt(step_g_loss_s), 4),
                         np.round(step_g_loss_v, 4), np.round(np.sqrt(step_e_loss_t0), 4)])
  ...
  # Save loss values to CSV files
  pd.DataFrame(embedding_losses, columns=['Iteration', 'Embedding Loss']).to_csv('embedding_losses.csv', index=False)
  pd.DataFrame(supervised_losses, columns=['Iteration', 'Supervised Loss']).to_csv('supervised_losses.csv', index=False)
  pd.DataFrame(joint_losses, columns=['Iteration', 'Discriminator Loss', 'Generator Loss (U)', 'Generator Loss (S)',
                                      'Generator Loss (V)', 'Embedding Loss (T0)']).to_csv('joint_losses.csv',
                                                                                           index=False)

  print("Loss data saved to CSV files.")
  ...
  flattened_data = np.reshape(generated_data, (-1, generated_data[0].shape[-1]))
  np.savetxt('generated_data.csv', flattened_data, delimiter=',')
