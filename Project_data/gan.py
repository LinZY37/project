...
for epoch in range(epochs):
    ...
    #     # 计算判别器准确率
    #     real_samples = next(iter(dataloader))[0][:500]
    #     fake_samples = torch.cat(list(gen_samples_queue))[:500]

    #     combined = torch.cat([real_samples, fake_samples])
    #     labels = torch.cat([torch.ones(500), torch.zeros(500)])

    #     with torch.no_grad():
    #         preds = (discriminator(combined) > 0.5).float()
    #         acc = (preds == labels).float().mean()

    # print(f"D准确率：{acc.item():.2%} | G损失：{avg_loss_g:.4f} | D损失：{avg_loss_d:.4f}")
...
def generate_sequences(num_sequences):
    ...
    with torch.no_grad():
        ...
        # 转换为 [num_sequences*seq_length, 3]
...
# 保存数据
generated_df.to_excel('generated_sequences.xlsx', index=False)
