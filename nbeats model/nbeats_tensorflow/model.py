...
class NBeats():
    ...
    def forecast(self, y, return_backcast=False):
        # 转换并校验输入数据
        y_original = cast_target_to_array(y)
        if len(y_original) < self.lookback_period:
            raise ValueError(f"Input y must have at least {self.lookback_period} observations.")

        # 归一化处理
        y_scaled = (y_original - self.y_min) / (self.y_max - self.y_min)

        # 模型预测
        backcast, forecast = self.model(y_scaled[-self.lookback_period:].reshape(1, -1))
        backcast_original = self.y_min + (self.y_max - self.y_min) * backcast[-1, :].numpy().flatten()
        forecast_original = self.y_min + (self.y_max - self.y_min) * forecast[-1, :].numpy().flatten()

        # 关键修正1: 时间轴长度基于输入数据计算
        total_length = len(y_original) + self.forecast_period  # 输入12条 + 预测1条 → 13
        df = pd.DataFrame(
            index=np.arange(total_length),
            columns=['actual', 'forecast', 'backcast']
        )
        df.index.name = 'time_idx'

        # 关键修正2: 使用传入的y_original填充actual列
        df['actual'].iloc[:len(y_original)] = y_original

        # 关键修正3: 预测值位置对齐到输入数据末尾之后
        df['forecast'].iloc[-self.forecast_period:] = forecast_original

        # 关键修正4: Backcast位置对齐到输入数据尾部
        if return_backcast:
            backcast_start = len(y_original) - self.lookback_period  # 输入数据末尾向前推lookback_period
            backcast_end = len(y_original)
            df['backcast'].iloc[backcast_start:backcast_end] = backcast_original
